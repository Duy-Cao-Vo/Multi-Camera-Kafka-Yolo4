...
class BaseCamera:
    ...
    def __init__(self, feed_type, topic, server_list):
        """Start the background camera thread if it isn't running yet."""
        self.unique_name = (feed_type, topic)
        ...
        if BaseCamera.threads[self.unique_name] is None:
            ...
            BaseCamera.threads[self.unique_name] = threading.Thread(target=self._thread,
                                                                    args=(self.unique_name, server_list))
    ...
    @staticmethod
    def server_frames(consumer, server):
        """"Generator that returns frames from the camera."""
        raise RuntimeError('Must be implemented by subclasses')
    ...
    @classmethod
    def server_thread(cls, unique_name, server):
        device = unique_name[1]
        frames_iterator = cls.server_frames(device, server)
        try:
            for cam_id, frame in frames_iterator:
                ...
                if time.time() - BaseCamera.last_access[unique_name] > 5:
                    ...
                    print('Closing server socket at port {}.'.format(server))
    ...
    @classmethod
    def _thread(cls, unique_name, server_list):
        feed_type, topic = unique_name
        BaseCamera.threads[unique_name] = None

        if feed_type == 'camera':
            port = server_list[int(topic)]
            print('Starting server thread for topic {} at port {}.'.format(topic, port))
            cls.server_thread(unique_name, port)

        elif feed_type == 'yolo':
            """Camera background thread."""
            port = server_list[int(topic)]
            print('Starting YOLO thread for device {} at port {}'.format(topic, port))
